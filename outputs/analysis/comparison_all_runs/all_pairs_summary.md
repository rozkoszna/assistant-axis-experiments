...
The EP run adds an "I am {trait}." prefix to every NQ prompt, making the target trait explicitly stated rather than only implicit in task context. This single modification substantially shifts which personality axes are activated: EP produces significantly more style-heavy expression, with traits like `casual` gaining a huge composite-score boost (NQ 96.9 → EP 160.3) and `playful` rising from 144.5 to 190.1. Lexical-style axes respond most strongly to the explicit signal — `concise` jumps from rank 50 (NQ) to rank 25 (EP), a gain of +73 significant axes. By contrast, epistemic and cognitive traits are narrowed by the prefix: `factual` loses 45 significant axes (NQ 118 → EP 73), suggesting the blunt self-declaration crowds out nuanced cognitive adjustment. Overall significance rates are similar (NQ 69.2 %, EP 71.7 %), but the *content* of what changes differs substantially.

The new axis-footprint overlap analysis (`nq_ep_axis_footprint_overlap.csv`) shows that the explicit-prefix effect is not only about breadth. For already-legible expressive traits, the same axes mostly remain significant: `entertaining` has 161 shared significant axes out of a 172-axis union, `playful` 163/175, `humble` 147/163, and `anxious` 143/167, with 100% direction agreement on shared axes. For label-sensitive traits, the axis identity changes more strongly: `concise` expands from 61 to 134 axes but shares only 47 axes across NQ/EP, gaining 87 and losing 14; `factual` contracts from 118 to 73 axes, sharing 65/126 and losing 53 formerly significant axes. The explicit prefix is therefore best understood as a label-sensitivity manipulation: it often preserves direction where axes overlap, but it changes which axes enter or leave the footprint.
...
|---|---|---|---|---|---|
| NQ vs EP | 69.2 % | 71.7 % | `factual` (+45 axes in NQ) | `concise` (+73 axes in EP) | Implicit style vs explicit-label footprint |
| NQ vs Opinion | 69.2 % | 71.2 % | `patient`, `accessible` | `data_driven`, `inspirational` | Task-execution style vs value expression |
| NQ vs Identity | 69.2 % | 53.4 % | `anxious`, `humble`, `reactive` | `educational`, `verbose` | Social mirroring vs cognitive anchoring |
| EP vs Opinion | 71.7 % | 71.2 % | `concise` | `data_driven` | Lexical register vs ideological stance |
| EP vs Identity | 71.7 % | 53.4 % | `playful`, `casual` | `educational`, `narrative` | Style performance vs structural elaboration |
| Opinion vs Identity | 71.2 % | 53.4 % | `data_driven`, `inspirational` | `educational`, `formal` | Value expression vs defensive elaboration |
