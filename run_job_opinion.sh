#!/usr/bin/env bash
# Non-interactive RunAI job: opinion_prompt_intents x user_plausible_traits_strict
...
runai-rcp-prod submit opinion-run \
  ...
  --command -- /bin/bash -lc '
    ...
    uv run python project/run_multi_trait_analysis.py \
      ...
      --comparison-name opinion_all_axes_llama \
      --intents-file data/opinion_prompt_intents.jsonl \
      --num-candidates 12 \
