...
This script reuses the projection logic from project_prompt_axis.py:
- load_model
- text_vector
- project
...
Notes:
- This script is best aligned with axes built using:
...
from project.project_prompt_axis import load_model, text_vector, project


def parse_args():
    parser = argparse.ArgumentParser(
        description="Project a pair of texts onto all trait axes."
    )
    parser.add_argument(
        "--axes-dir",
        type=str,
...
def load_axis_vector(path: Path, layer: int):
    ...
    if vectors.ndim != 2:
        raise ValueError(
            f"Expected 'vector' in {path} to have shape [n_layers, d_model], got {tuple(vectors.shape)}"
        )

    if not (0 <= layer < vectors.shape[0]):
        raise ValueError(
            f"Layer {layer} out of range for {path}; vector stack shape is {tuple(vectors.shape)}"
        )
