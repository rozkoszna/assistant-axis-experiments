...
class SteeredModel:
        ...
        for layer_idx in self.steer_layers:
            ...
            def make_patched(orig_fwd, sv, a, mode, tau, patched_layer_idx):
                def patched(*args, **kwargs):
                    ...
                    if hidden is None:
                        if self._debug_active:
                            self._debug_stats[patched_layer_idx]["calls"] += 1.0
                        return output
    ...
    def generate_unsteered(
        ...
        """Baseline: no activation steering."""
        device = next(self.model.parameters()).device
        input_ids, attention_mask = self._tokenize_for_generation(
            self._build_input(prompt, system_prompt), device
        )
    ...
    def generate_unsteered_batch(
        ...
        """Generate a batch of responses without activation steering."""
...
def main():
    ...
    if args.compare:
        ...
        print("UNSTEERED:")
        print("=" * 60)
        print(teacher.generate_unsteered(args.prompt))

    print("\n" + "=" * 60)
    print(f"STEERED (alpha={args.alpha}, layers={teacher.steer_layers}, persona={teacher.persona}):")
    print("=" * 60)
    print(teacher.generate(args.prompt, system_prompt=args.system_prompt,
